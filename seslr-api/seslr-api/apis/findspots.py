...
find = api.model('Find', {
    ...
    'features_architecture': fields.List(
        fields.String(),
        description='The architectural features of the find.'
    ),
    'features_sepulchral': fields.List(
        fields.String(),
    ...
    'material_bone': fields.List(
        fields.String(),
        description='The bone material of the find.'
    ),
    'material_building': fields.List(
        fields.String(),
...
class FindSpot(Resource):
    ...
    def get(self, find_spot_id):
        ...
        query = """
        SELECT toponym, type2, description
        FROM seslr.find_spot
        WHERE "find_spot_ID" = %s;
        """
        ...
        if find_spot_info is None:
            raise NotFound(
                'No spots with id {} found in database.'.format(find_spot_id)
            )
        ...
        WITH coltorows AS (
            SELECT
                "find_spot_ID",
                UNNEST(
                    ARRAY['LN', 'FN', 'N', 'EBA', 'MBA', 'LBA', 'EPG',
                          'MPG', 'LPG', 'PG', 'EG', 'MG', 'LG', 'G', 'EA',
                          'MA', 'LA', 'A', 'EC', 'MC', 'LC', 'C', 'EH',
                          'MH', 'LH', 'H', 'ER', 'MR', 'LR', 'R', 'EB',
                          'MB', 'LB', 'B', 'F', 'O', 'M', 'xPh', 'xFN',
                          'xN', 'xEBA', 'xMBA', 'xLBA', 'xEPG', 'xMPG',
                          'xLPG', 'xPG', 'xEG', 'xMG', 'xLG', 'xG', 'xEA',
                          'xMA', 'xLA', 'xA', 'xEC', 'xMC', 'xLC', 'xC',
                          'xEH', 'xMH', 'xLH', 'xH', 'xER', 'xMR', 'xLR',
                          'xR', 'xEB', 'xMB', 'xLB', 'xB', 'xF', 'xO', 'xM']
                ) AS col_name,
                UNNEST(
                    ARRAY["LN", "FN", "N", "EBA", "MBA", "LBA", "EPG",
                          "MPG", "LPG", "PG", "EG", "MG", "LG", "G", "EA",
                          "MA", "LA", "A", "EC", "MC", "LC", "C", "EH", "MH",
                          "LH", "H", "ER", "MR", "LR", "R", "EB", "MB", "LB",
                          "B", "F", "O", "M", "xPh", "xFN", "xN", "xEBA",
                          "xMBA", "xLBA", "xEPG", "xMPG", "xLPG", "xPG", "xEG",
                          "xMG", "xLG", "xG", "xEA", "xMA", "xLA", "xA", "xEC",
                          "xMC", "xLC", "xC", "xEH", "xMH", "xLH", "xH", "xER",
                          "xMR", "xLR", "xR", "xEB", "xMB", "xLB", "xB", "xF",
                          "xO", "xM"]
                ) AS col_value
            FROM seslr.find_spot_chronology
        )
        SELECT array_agg(col_name) AS true_col_names
        FROM coltorows
        WHERE col_value
        AND "find_spot_ID" = %s
        GROUP BY "find_spot_ID";
        """
...
class Find(Resource):
    ...
    @api.doc(params={"find_spot_id": "The id of the find spot"})
    @api.marshal_with(find,
                      mask=('description, features, features_architecture, '
                            'features_sepulchral, material, material_bone, '
                            'material_building'))
    def get(self, find_spot_id):
        ...
        WITH coltorows AS (
            SELECT
                find_spot_id,
                UNNEST(
                    ARRAY['architecture', 'floors', 'pavement',
                          'stone_alignment', 'remotely_sensed_features',
                          'rock_cuttings', 'sepulchral', 'negative_feature']
                ) AS col_name,
                UNNEST(
                    ARRAY["architecture", "floors", "pavement",
                          "stone_alignment", "remotely_sensed_features",
        ...
        SELECT array_agg(col_name) AS true_col_names
        FROM coltorows
        WHERE col_value AND find_spot_id = %s
        GROUP BY find_spot_id;
        """
        ...
        WITH coltorows AS (
            SELECT
                find_spot_id,
                UNNEST(
                    ARRAY['enclosure_walls', 'terrace_walls',
                          'foundation_walls', 'defensive_walls',
                          'building_walls', 'wall', 'retaining_walls',
                          'other']
                ) AS col_name,
                UNNEST(
                    ARRAY["enclosure_walls", "terrace_walls",
        ...
        WITH coltorows AS (
            SELECT
                find_spot_id,
                UNNEST(
                    ARRAY['pottery', 'glass', 'metal', 'lithics', 'various',
                          'ceramic_building_material', 'botanical', 'bone',
                          'slag']
                ) AS col_name,
                UNNEST(
                    ARRAY["pottery", "glass", "metal", "lithics", "various",
        ...
        WITH coltorows AS (
            SELECT
                find_spot_id,
                UNNEST(
                    ARRAY['tile', 'brick', 'spolia', 'mortar', 'blocks',
                          'rubble', 'arch_members']
                ) AS col_name,
                UNNEST(
                    ARRAY["tile", "brick", "spolia", "mortar", "blocks",
        ...
        info = [description, features, features_architecture,
                features_sepulchral, material, material_bone,
                material_building]
        if all(value is None for value in info):
            raise NotFound(
                'No finds with id {} found in database.'.format(find_spot_id)
            )
        else:
            return {
                "id": find_spot_id,
