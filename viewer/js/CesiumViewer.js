  ...
  feature
    ...
    .then(function(feature) {
      ...
      if (featureName.id === 'find_spots') {
        ...
        for (const entity of feature.entities.values) {
          const survey = Cesium.Property.getValueOrUndefined(entity.properties.arch_proje);
          entity.polygon.material = Cesium.Color.fromCssColorString(surveyColors[survey]);
        }
      } else if (featureName.id === 'geology') {
        for (const entity of feature.entities.values) {
          const geologyUnit = Cesium.Property.getValueOrUndefined(entity.properties.geology_lo);
          entity.polygon.material = Cesium.Color.fromCssColorString(geologyColors[geologyUnit]);
        }
      }
    });
  feature.show = false;
